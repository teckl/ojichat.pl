...
sub select_pattern {
    my $patterns = [
        # GQS パターン
        OnaraPattern->new('GREETING', 'QUESTION', 'SYMPATHY'),
        # GR パターン
        OnaraPattern->new('GREETING', 'REPORTING'),
        # GC パターン
        OnaraPattern->new('GREETING', 'CHEERING'),
        # GQI パターン
        OnaraPattern->new('GREETING', 'QUESTION', 'INVITATION'),
        # PA パターン
        OnaraPattern->new('PRAISING', 'ADMIRATION'),
        # S パターン (短いので SS にする)
        OnaraPattern->new('SYMPATHY', 'SYMPATHY'),
    ];
    return @{$patterns}[int rand scalar @{$patterns}];
};

sub select_template {
    my ($class, $emotion) = @_;
    if ($emotion eq 'GREETING') {
        [
